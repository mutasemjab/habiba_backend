...
@section('title')
    {{ __('messages.send_notification_to_client') }}
@endsection
@section('navbar')
        ...
        <div class="col-lg-12 mb-4">
            <div class="card shadow mb-4">
                <div class="card-header py-3 d-flex justify-content-between align-items-center">
                    <h6 class="m-0 font-weight-bold text-primary">{{ __('messages.send_notification_to_client') }}</h6>
                </div>
                <div class="card-body">
                    @include('layouts.errors')
                    <form action="{{ route('send_client_notification') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group">
                            <label for="sub_categories">{{ __('messages.select_client') }}</label>
                            <select name="client_id" id="" class="form-control">
                                <option value="" disabled hidden selected>{{ __('messages.select_client') }}</option>
